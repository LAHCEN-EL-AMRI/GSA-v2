...
public class treatmentLog extends JPanel {	
   ...
   // static ArrayList<String> ForActivity;
	...
	static String resultsfile="";
	       ...
	       String contenuArefile="      Channel"+epsfile+"Energy"+epsfile+"FWHM"+epsfile+"Net area"+epsfile+"Background"+epsfile+"Error (%)"+epsfile+"Activity";                                           
	...
	public treatmentLog() throws IOException {
        ...
        //ForActivity               = new ArrayList<String>(); //pour activity et efficacité 
      ...
      //----------------------------------------------------------
        ...
        for(j4=0;j4<mariscoti.ChannelLissee.size();j4++){
        	...
        	for(ia=0;ia<xs_m.size()-1;ia++){
				                            ...
				                            if(jcc==0 && Pg>7*ecartc){//3*ecart pour éviter la premier dessigne en graphe				
				                                      ...
				                                      for(i1m=0;i1m<yd.size()-1;i1m++){
				                                          if(i1m<=ia && i1m>=jg){ 
	        		                                             ...
	        		                                             int ig;
	        		                                             float ecarti,enrgi,talli,withi, EffiCal;
	        		                                             ...
	        		                                             EffiCal=area_net_gaus;
	        		                                             
	        		                                             //EffiCal=(float)(1 + 2 * Math.log(enrgi) + 3 * Math.pow(Math.log(enrgi), 2) + 4 * Math.pow(Math.log(enrgi), 3)+ 5 * Math.pow(Math.log(enrgi), 4) + 6 * Math.pow(Math.log(enrgi), 5));
	        		                                             ...
	        		                                             //ForActivity.add(Float.toString(EffiCal));
				...
				int is=0, withj;
				float ecartj = 0,enrgj = 0,enrgj1,tallj = 0,tallj1, EffiCalj=0;
				...
				ll=ll+1;
				//EffiCalj=enrgj;
				//EffiCalj=(float)(1 + 2 * Math.log(enrgj) + 3 * Math.pow(Math.log(enrgj), 2) + 4 * Math.pow(Math.log(enrgj), 3)+ 5 * Math.pow(Math.log(enrgj), 4) + 6 * Math.pow(Math.log(enrgj), 5));
				...
				resultfile=resultfile+lin+tr.format(ll)+"S "+"   "+df0.format(Xa)+espacefile+df0.format(enrgj)+espacefile+df0.format(ecartj)+espacefile+df0.format(Area_net)+espacefile+df0.format(backgrounds)+espacefile+df0.format(eror);
                 ...
                 //ForActivity.add(Float.toString(EffiCalj));
